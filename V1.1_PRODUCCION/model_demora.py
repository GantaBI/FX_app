...
train_data = df[['Sexo', 'Edad', 'Lugar de residencia', 'Movilidad PreFractura',
       ...
       'Tipo de Fractura',      'Sedest_postQx', 'UPP.intrahosp', 'Leucocitos', 'Glucosa',
...
train_labels = df['Demora_Qx'][:].values.tolist()
...
cat_features = [ 0, 2, 3, 6, 7,8, 9, 18]



# Initialize CatBoostRegressor
model_demora = CatBoostRegressor(iterations=300,
                           learning_rate=0.03,
                           depth=5, custom_metric = 'MAE', loss_function='RMSE', l2_leaf_reg=0.1, early_stopping_rounds=10)
# Fit model
model_demora.fit(train_data, train_labels, cat_features)
# Get predictions
preds = model_demora.predict(eval_data)
...
# %%
model_demora.predict(['Mujer', 89, 'Residencia', 3, 5, 3, 'Izquierda', 'Pertrocantérea', '1.0', '0.0',
...
model_demora.save_model("model_ds_demora_dias_rnfc_4")
# %%
